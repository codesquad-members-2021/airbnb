...
extension ConditionViewModel {
    ...
    func finalCondition() -> String {
        let city = convertedCity()
        let peopl = people!.1 != 0 ? "게스트 \(people!.0)명 유아 \(people!.1)명" : "게스트 \(people!.0)명"
        return "\(city) ・ \(schedule!.description) ・ \(peopl)"
    }
    
    func convertedCity() -> String {
        let cityDic = [1: "서울", 2: "경기도", 3: "강원도", 4: "충청도", 5: "경상남도", 6: "경상북도", 7: "전라남도", 8: "전라북도"]
...
struct Schedule: Codable {
    ...
    var description: String {
        let checkin = checkIn == nil ? "" : "\(checkOut!.month)월 \(checkIn!.day)일"
        let checkout = checkOut == nil ? "" : "\(checkOut!.month)월 \(checkOut!.day)일"
