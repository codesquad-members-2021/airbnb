...
import com.example.airbnb.domain.Reservation;
...
public class ReservationController {
    ...
    public Object reserveRoom(@ApiParam("방방 식별자") @PathVariable Long roomId,
        ...
        if (reservationService.checkPeriod(roomId, checkIn, checkOut)) {
            return ResponseEntity.badRequest().body("선택하신 기간에 예약 불가능합니다.");
        }
        if (reservationService.checkNumGuest(roomId, numOfPeople)) {
    ...
    @ApiOperation(value = "예약 취소", notes = "예약 취소합니다.")
    public Object deleteReservation(@ApiParam("예약 식별자") @PathVariable Long reservationId) {
